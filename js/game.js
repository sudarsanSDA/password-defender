document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Game Config & State ---
    const WAVES_PER_LEVEL = 3, MAX_ATTEMPTS = 3;
    const LEVELS = [
    ...
    const handleAttackOutcome = (isSuccess) => {
        if (isSuccess) {
            ...
            if (failedAttempts < MAX_ATTEMPTS) { setTimeout(startNextWave, 2000); } 
    ...
    const showScreen = (screenName) => {
        ...
        Object.values(screens).forEach(s => {
            if (s.classList.contains('overlay')) s.classList.remove('visible');
        });
        // Hide the main game screen container
        screens.game.classList.add('hidden');
    
        if (screenName === 'game') {
            // Only show the game screen if that's the target
            screens.game.classList.remove('hidden');
        } else {
            // Otherwise, show the target overlay
            screens[screenName].classList.add('visible');
        }

        if (screenName === 'mainMenu') renderMainMenu();
    };
    ...
    // --- VFX & Animations ---
    const fireLasers = (isSuccess) => { document.querySelectorAll('.hacker').forEach(hacker => { const cR=castleContainer.getBoundingClientRect(), cC={x:cR.left+cR.width/2,y:cR.top+cR.height/2}, hR=hacker.getBoundingClientRect(), hC={x:hR.left+hR.width/2,y:hR.top+hR.height/2}; const dX=cC.x-hC.x,dY=cC.y-hC.y,dist=Math.sqrt(dX*dX+dY*dY),ang=Math.atan2(dY,dX)*180/Math.PI,l=document.createElement('div');l.className='laser-beam';l.style.cssText=`left:${hC.x}px;top:${hC.y}px;width:${dist}px;transform:rotate(${ang}deg);`;gameWorld.appendChild(l);if(!isSuccess){const s=document.createElement('div');s.className='impact-spark';s.style.cssText=`left:${cC.x}px;top:${cC.y}px;`;gameWorld.appendChild(s);setTimeout(()=>s.remove(),400)}setTimeout(()=>l.remove(),400)});};
